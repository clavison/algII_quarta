...
public class PrincipalProduto {
	...
	public static void main(String[] args) {
		...
		do {
			op = menu();
			if(op == 1) lista.add(leProduto());
			if(op == 2) buscarNome(lista);
			if(op == 3) totalVendasMes(lista);
			if(op == 4) mostraEstoque(lista);
		}while(op != 5);
	...
	public static void buscarNome(ArrayList<Produto> lista) {
	...
	public static void totalVendasMes(ArrayList<Produto> produtos) {
		...
		double soma = 0;
		for(Produto p: produtos) {
			soma += p.vendas[mesBusca-1]*p.valor;
		}
		msg(soma+"");
	}
